...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'Niche Construction <onboarding@resend.dev>',
      to: email,
      subject: `You've been invited to "${projectName}" on Niche Construction`,
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
          <div style="background: #1a1a1a; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
            <h1 style="color: white; margin: 0; font-size: 20px;">Niche Construction</h1>
          ...
          <p style="color: #555; line-height: 1.6;">
            ...
            <strong>"${projectName}"</strong> on Niche Construction.
          </p>
          <p style="color: #555; line-height: 1.6;">
